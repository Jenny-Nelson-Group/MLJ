...
for i in [1, 2, 3]:
    ...
    for x in test_range:
        ground_state = State(
            energy=0.0, disorder_number_of_states=3, disorder_sigma=0.1
        )
        ...
        match situation:
            case 1:
                excited_state = State(
                    energy=1.5,
                    disorder_number_of_states=nr_fix,
                    disorder_sigma=x,
                    disorder_integration_cut_off=cut_off_fix,
                    disorder_distribution=distr,
                )
            case 2:
                excited_state = State(
                    energy=1.5,
                    disorder_number_of_states=x,
                    disorder_sigma=sigma_fix,
                    disorder_integration_cut_off=cut_off_fix,
                    disorder_distribution=distr,
                )
            case 3:
                excited_state = State(
                    ...
                    disorder_number_of_states=nr_fix,
                    disorder_sigma=sigma_fix,
                    disorder_integration_cut_off=x,
                    disorder_distribution=distr,
                )
        ...
        Znorm_abs = partition_function(
            transition=transition, process=ProcessType.RECOMBINATION,
    ...
    match situation:
        case 1:
            plt.xlabel("sigma")
            plt.title(
                f"sigma = changes, nr of states = {nr_fix}, cut off = {cut_off_fix}, "
            )
            plt.ylim(0, 5)
        case 2:
            plt.xlabel("number of states")
            plt.title(
                f"sigma = {sigma_fix}, nr of states = change, cut off = {cut_off_fix}"
            )
        case 3:
            plt.xlabel("cut off")
            plt.title(
                f"sigma = {sigma_fix}, nr of states = {nr_fix}, cut off = changes"
            )
