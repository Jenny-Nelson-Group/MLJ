...
def test_single_le_state_population():
    ...
    # Assertions
    assert pop.size == 1, "Should return a single float for one state"
...
def test_le_ct_two_state_weighting():
    ...
    # Run
    states = [le_state, ct_state]
    results = states_dark_population(states, weights=custom_weights)

    # Assertions
    assert isinstance(results, np.ndarray)
    assert results.ndim == 2  # (L, M)
    assert results.shape[0] == len(states)
