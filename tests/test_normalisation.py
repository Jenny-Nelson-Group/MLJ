...
def test_output_format_type():
    ...
    Znorm_abs = partition_function(transition=transition, temperatures=np.array([200,300]), process=ProcessType.ABSORPTION)
    Znorm_rec = partition_function(transition=transition, temperatures=np.array([200,300]), process=ProcessType.RECOMBINATION)
