...
def sample_transition(request):
    ...
    le = State(
        name="LE",
        energy=gs_le_energy_gap,
        vib_spacing=vibrational_spacing,
        disorder_number_of_states=n_states,
    )

    return Transition(
        state_low_energy=gs, state_high_energy=le, lambda_inner=0.15, lambda_outer=0.05
    )
...
def test_fcwd_output_shape(sample_transition):
    ...
    fcwd_spectrum = fcwd(
        photon_energies, sample_transition, temperatures=temps, process=ProcessType.ABSORPTION
    )
...
def test_temperature_broadening(sample_transition):
    ...
    fcwd_spectrum_cold = fcwd(
        photon_energies,
        sample_transition,
        temperatures=np.array([50.0]),
        process=ProcessType.ABSORPTION,
    )[:, 0, 0]
    fcwd_spectrum_hot = fcwd(
        photon_energies,
        sample_transition,
        temperatures=np.array([500.0]),
        process=ProcessType.ABSORPTION,
    )[:, 0, 0]

    assert np.max(fcwd_spectrum_cold) > np.max(
        fcwd_spectrum_hot
    ), "Cold peak should be sharper and higher than hot peak"
...
def test_zero_vibrational_levels(sample_transition, request):
    ...
    fcwd_spectrum = fcwd(
        photon_energies, sample_transition, temperatures, process=ProcessType.ABSORPTION
    )
...
def test_abs_vs_rec_peak_shift(sample_transition):
    ...
    # Case 1: Absorption
    fcwd_spectrum_abs = fcwd(
        photon_energies, sample_transition, temperatures, process=ProcessType.ABSORPTION
    )[:, 0, 0]
    peak_abs = photon_energies[np.argmax(fcwd_spectrum_abs)]

    # Case 2: Recombination
    fcwd_spectrum_rec = fcwd(
        photon_energies, sample_transition, temperatures, process=ProcessType.RECOMBINATION
    )[:, 0, 0]
...
def test_huang_rhys_factor_ratio(lambda_inner):
    ...
    transition = Transition(
        state_low_energy=ground_state, state_high_energy=excited_state, lambda_inner=lambda_inner
    )

    # Sanity Check: Verify the Transition calculates the correct S
    assert np.isclose(
        transition.huang_rhys, huang_rhys_target
    ...
    photon_energies = np.linspace(
        gs_le_energy_gap - 0.5, gs_le_energy_gap + 5 * vibrational_spacing, resolution
    )
    fcwd_spectrum_abs = fcwd(
        photon_energies, transition, temperatures, process=ProcessType.ABSORPTION
    )
    ...
    if intensity_value_00_peak is None or intensity_value_01_peak is None:
        pytest.fail(
            f"Could not resolve 0-0 and 0-1 peaks for S={huang_rhys_target}. Check resolution or range."
        )
    ...
    assert np.isclose(
        huang_rhys_calculated, huang_rhys_target, rtol=0.05
...
def test_fcwd_integral_normalization(sample_transition):
    ...
    This checks if the Gaussian broadening and Boltzmann weights are normalised.
    """
    ...
    # Test Absorption
    y_abs = fcwd(photon_energies, sample_transition, temperatures, process=ProcessType.ABSORPTION)[
        :, 0, 0
    ]
...
def get_i00_i01_intensities(spectrum_intensity):
    ...
    peaks, properties = find_peaks(
        spectrum_intensity, height=0.1 * np.max(spectrum_intensity), distance=5
    )
    ...
    intensity_value_00_peak = spectrum_intensity[
        peak_indices[0]
    ]  # Intensity of the first peak (0-0)
    intensity_value_01_peak = spectrum_intensity[
        peak_indices[1]
