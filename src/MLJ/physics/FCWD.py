...
def fcwd(photon_energies, transition):
    ...
    vib_spacing = transition.state_high_energy.hW        # vibrational quantum (hΩ) (eV)
    ...
    v_i_mat, v_f_mat, photon_energies_mat, gibbs_energies_mat, temperature_mat = np.meshgrid(v_i, v_f, photon_energies, gibbs_energies, temperature, indexing='ij')
    ...
    factor2 = (np.exp(-(-sign*photon_energies_mat + sign*gibbs_energies_mat + outer_reorganisation_energy + vib_diff * vib_spacing) ** 2 
            / (4 * outer_reorganisation_energy * const.CONST_kB * temperature_mat)))

    # Boltzmann population of the initial state
    factor3 = np.exp(-(v_i_mat * vib_spacing) / (const.CONST_kB * temperature_mat))

    normalisation = 1 / np.sqrt(4 * np.pi * outer_reorganisation_energy * const.CONST_kB  * temperature_mat)
